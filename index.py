...
def qa_program(extracted_keywords, pos_question,words,docs,inverted_index):
    terms =[]
    for term in extracted_keywords:
        if term in words:
            terms.append(term)
    if not term:
        return pp('Please give more informations')
...
answer = qa_program(extracted_keywords, pos_question,words,docs,inverted_index)